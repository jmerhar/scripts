...
A generic package-file generator designed to be run from a CI/CD workflow. It takes the path to a script as an argument, parses metadata from a structured comment block at the top of the script (between `# --- SCRIPT INFO START ---` and `# --- SCRIPT INFO END ---` markers), and generates the necessary package files (`.rb` for Homebrew and `.deb` for Debian).
...
* `PKGSCR_HOMEBREW_FORMULA_DIR`: The local output directory for the generated Homebrew formula files.
* `PKGSCR_DEB_PACKAGE_DIR`: The local output directory for the generated Debian package files.
* `PKGSCR_CONFIG_DIR`: The directory where source config files are located.
* `PKGSCR_TARBALL_URL`: The URL to the tarball of the new release.
* `PKGSCR_VERSION`: The version string (e.g., `v1.0.1`).
* `PKGSCR_SHA256_CHECKSUM`: The pre-calculated checksum of the release tarball.
